...
from utr.data.downstream.secondary_structure.datamodule import SecondaryStructureDataModule, SUPPORTED_DATASETS
...
class SecStructPredictionWrapper(pl.LightningModule):
    ...
    def load_pretrained_lm_weights(self, pretrained_weights_path):#需要修改
        #self.lm.load_state_dict(torch.load(pretrained_weights_path))
        #self.lm.load_from_checkpoint(checkpoint_path=args.pretrained_rinalmo_weights)
    ...
    def _eval_step(self, batch, batch_idx, thresholds, log_prefix="eval"):
        ss_ids, seqs, _, sec_struct_true = batch   # 、原始序列、编码后的原始序列、二级结构的数量表示
...
def main(args):
    ...
    if args.pretrained_rinalmo_weights:
        ...
        #model = SecStructPredictionWrapper.load_from_checkpoint(checkpoint_path=args.pretrained_rinalmo_weights)
