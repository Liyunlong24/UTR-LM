...
from utr.utils.tensor_utils import tensor_tree_map
...
class ExponentialMovingAverage:
            ...
            for k, v in update.items():
                ...
                if not isinstance(v, torch.Tensor):
                    ...
                    if stored.dtype == torch.long:
                        # the stored value is an index or a constant, so we don't update it
                        continue
                        # diff = stored - v
                        # assert diff.sum() == 0
                    else:
    ...
    def load_state_dict_from_model(self, model: nn.Module, decay: Optional[float] = None) -> None:
