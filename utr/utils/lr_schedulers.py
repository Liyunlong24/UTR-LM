...
class AlphaFoldLRScheduler2(torch.optim.lr_scheduler._LRScheduler):
    def __init__(self, 
                 optimizer, 
                 last_epoch: int = -1, 
                 verbose: bool = False,
        ...
        super(AlphaFoldLRScheduler2, self).__init__(
            optimizer,
            last_epoch=last_epoch,
            verbose=verbose,
        )
    ...
    def get_lr(self):
        ...
        for group in self.optimizer.param_groups:
            if 'initial_lr' not in group:
                raise ValueError("Optimizer's param_groups must have 'initial_lr' key")
            max_lr = group.get('initial_lr')
            base_lr = self.base_lr

            if(step_no <= self.warmup_no_steps and self.warmup_no_steps > 0):
                lr = base_lr + (step_no / self.warmup_no_steps) * max_lr
            elif(step_no > self.start_decay_after_n_steps):
                steps_since_decay = step_no - self.start_decay_after_n_steps
                exp = (steps_since_decay // self.decay_every_n_steps) + 1
                lr = max_lr * (self.decay_factor ** exp)
            else: # plateau
                lr = max_lr
            lrs.append(lr)
