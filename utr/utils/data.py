...
from utr.utils.constants import proteinseq_toks, dnaseq_toks, rnaseq_toks
...
class Alphabet(object):
    ...
    def get_special_tokens_mask(self, idxs):
        mask = [self.get_tok(tok) not in self.standard_toks for tok in idxs]
...
class utrlm_BatchConverter(object):
    """Callable to convert an unprocessed (labels + strings) batch to a
    processed (labels + tensor) batch.
    """
    ...
    def __init__(self, alphabet, truncation_seq_length: int = None):
       ...
       # self.alphabet.append_eos = True
        self.truncation_seq_length = truncation_seq_length
        self.structure_tok_to_idx = {
            '(': 0,
            '.': 1,
            ')': 2,
        }
        self.alphabet.prepend_bos = self.alphabet.prepend_bos if hasattr(self.alphabet, 'prepend_bos') else True
        self.alphabet.append_eos = self.alphabet.append_eos if hasattr(self.alphabet, 'append_eos') else True

    def __call__(self, raw_batch: Sequence):
        # RoBERTa uses an eos token, while ESM-1 does not.
        batch_size = len(raw_batch)
        batch_inputs, batch_labels, batch_masked_indices, batch_input_desc = zip(*raw_batch)
        seq_encoded_list = batch_inputs
        label_seq_encoded_list = batch_labels

        if self.truncation_seq_length:
            seq_encoded_list = [seq_str[:self.truncation_seq_length] for seq_str in seq_encoded_list]
            label_seq_encoded_list = [seq_str[:self.truncation_seq_length] for seq_str in label_seq_encoded_list]
            batch_masked_indices = [seq_str[:self.truncation_seq_length] for seq_str in batch_masked_indices]
        max_len = max(len(seq_encoded) for seq_encoded in seq_encoded_list)
        ...
        for i, (seq_encoded, label_seq_encoded, masked_indices, input_desc) in enumerate(
            zip(seq_encoded_list, label_seq_encoded_list, batch_masked_indices, batch_input_desc)
        ):
            if self.alphabet.prepend_bos:
                tokens[i, 0] = self.alphabet.cls_idx
                # label_tokens[i, 0] = self.alphabet.cls_idx
                # mask_indices[i, 0] = 0
            seq = seq_encoded.to(torch.int64)
            label_seq = label_seq_encoded.to(torch.int64)
            tokens[
                i,
                int(self.alphabet.prepend_bos): len(seq_encoded)
                + int(self.alphabet.prepend_bos),
            ...
            label_tokens[
                i,
                int(self.alphabet.prepend_bos): len(label_seq_encoded)
                + int(self.alphabet.prepend_bos),
            ] = label_seq
            mask_indices[
                i,
                int(self.alphabet.prepend_bos): len(masked_indices)
                + int(self.alphabet.prepend_bos),
            ] = masked_indices
