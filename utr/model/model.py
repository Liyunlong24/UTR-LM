...
from utr.model.modules import Transformer, MaskedLanguageModelHead, TokenDropout
...
class RiNALMo(nn.Module):
    ...
    def forward(self, tokens, need_attn_weights=False):
        ...
        if self.config.model.transformer.use_flash_attn:#成立
        ...
        result = {"logits": x, "representation": representation}#这个为最终的返回结果

        if need_attn_weights:#不成立
