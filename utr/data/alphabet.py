...
class Alphabet:
    def __init__(
        ...
        self.padding_idx = self.tkn_to_idx[PAD_TKN]
        self.mask_idx = self.tkn_to_idx[MASK_TKN]
        self.prepend_bos = True  # 或者根据需要设置为 False
        self.append_eos = True  # 或者根据需要设置为 False
    ...
    def get_special_tokens_mask(self, idxs):
        mask = [self.get_tkn(tok) not in self.standard_tkns for tok in idxs]
